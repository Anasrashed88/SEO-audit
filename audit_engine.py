...
EXCLUDED_COLUMNS = ['الرابط', 'نوع الصفحة', 'الحالة', 'كود الاستجابة',
                    'وجهة التحويل', 'في خريطة الموقع', 'مصدر الاكتشاف']

EXCLUDED_LABEL = {
    '404': 'محذوفة (404)',
    '410': 'محذوفة (410)',
    REDIRECT_HOME: 'محوّلة للرئيسية',
    'noindex': 'مخفية عن جوجل (noindex)',
}

def _unreachable_record(url, base_url, source, code, dest=''):
    return {
        'page_data': {
            # النوع يُستنتج من الرابط حتى لو لم تُفتح الصفحة
            'نوع الصفحة': _detect_type_by_url(clean_url(url), base_url),
            'الرابط': url, 'مصدر الاكتشاف': source, 'وجهة التحويل': dest,
...
def audit_single_page(task, timeout=10):
    ...
    if url_key(clean_url(url)) != home_key:
        ...
        if url_key(final_url) == home_key or (canon_now and url_key(canon_now) == home_key):
            return _unreachable_record(url, base_url, source, REDIRECT_HOME, dest=final_url)
    ...
    return {
        'page_data': {
            ...
            'وجهة التحويل': final_url if url_key(final_url) != url_key(clean_url(url)) else '',
...
def run_full_audit(target_url, max_pages=1500, workers=8, progress_cb=None):
    ...
    # جدول الروابط المستبعدة: كل رابط وسبب استبعاده كما اكتشفته الأداة
    excluded_df = full_df[gone_mask | noindex_mask].copy()
    if not excluded_df.empty:
        excluded_df['الحالة'] = [
            EXCLUDED_LABEL.get('noindex') if (av and not ix)
            else EXCLUDED_LABEL.get(code, code)
            for code, av, ix in zip(excluded_df['كود الاستجابة'],
                                    excluded_df['متاحة'], excluded_df['قابلة للأرشفة'])
        ]
        excluded_df['في خريطة الموقع'] = excluded_df['_key'].isin(sitemap_keys).map(
            lambda x: 'نعم' if x else 'لا')
        excluded_df = excluded_df[EXCLUDED_COLUMNS]
    else:
        excluded_df = pd.DataFrame(columns=EXCLUDED_COLUMNS)
