...
def pick_font():
    """يعيد (الاسم، مسار العادي، مسار العريض أو العادي كبديل آمن)."""
...
def build_diagnosis(score, stats, lang):
    ...
    if lang == 'ar':
        ...
        if stats.get('title_symbols'):
            points.append(f"{stats['title_symbols']} عنوان ميتا مجرد رموز أو قيمة قالب "
                          "افتراضية بدل النص، أي أن الصفحة بلا عنوان فعلي في نتائج البحث.")
        ...
        if crit_t:
            points.append(f"{crit_t} عنوان ميتا يحتاج إصلاحاً عاجلاً: مفقود أو أقصر من "
                          f"{TITLE_MIN_OK} حرفاً أو يتجاوز {TITLE_MAX} حرفاً فيُقتطع في نتائج البحث.")
        if imp_t:
            points.append(f"{imp_t} عنوان ضمن الحد المقبول ويمكن رفعه إلى الطول المثالي "
                          f"({TITLE_MIN_OPTIMAL}-{TITLE_MAX} حرفاً) لاستغلال كامل المساحة المعروضة.")
        if crit_d:
        ...
        if stats.get('url_clone'):
            points.append(f"{stats['url_clone']} منتجاً مستنسخاً: رابطه يحمل بادئة النسخ "
                          "أو لاحقة رقمية، وهي نسخ مكررة من منتج واحد تتنافس مع أصلها في نتائج البحث.")
        if stats.get('dup_content'):
            points.append(f"{stats['dup_content']} صفحة تتشارك نفس العنوان والوصف حرفياً، "
                          "فتُعدّ محتوى مكرراً ويختار جوجل واحدة ويتجاهل الباقي.")
        if stats.get('url_wrongname'):
            points.append(f"{stats['url_wrongname']} رابط يحمل اسم منتج مختلف عن المنتج المعروض "
                          "في الصفحة، غالباً لأن المنتج نُسخ ثم غُيّر اسمه دون تحديث الرابط.")
        ...
        if stats.get('url_malformed'):
            points.append(f"{stats['url_malformed']} رابط معطوب يحتوي عنوان موقع داخل مساره، "
                          "فيظهر للزبون في نتائج البحث بشكل مشوّه ويضعف الثقة.")
        ...
        if stats.get('img_legacy') and stats.get('img_modern_pct', 100) < 50:
            points.append(f"{stats['img_legacy']} صورة بصيغة قديمة ثقيلة بدل الصيغ الحديثة الخفيفة.")
        if stats.get('canon_broken'):
            points.append(f"{stats['canon_broken']} صفحة تشير بوسم الكانونيكال إلى صفحة واحدة بدل نفسها.")
        ...
        if stats.get('dead_count'):
            points.append(f"{stats['dead_count']} رابط محذوف ما زال معلناً في خريطة الموقع.")
        if stats.get('hidden_count'):
            points.append(f"{stats['hidden_count']} صفحة منشورة في خريطة الموقع لا يصل إليها الزائر بأي رابط داخلي.")
        if stats.get('unlisted_count'):
            points.append(f"{stats['unlisted_count']} صفحة معروضة في المتجر وغير مدرجة في خريطة الموقع.")
        if stats.get('scroll_only_count'):
            points.append(f"{stats['scroll_only_count']} منتجاً لا يظهر رابطه إلا بالتمرير فقط.")
        ...
        if score < 60:
            close = ("تشير النتيجة الإجمالية إلى فجوة واسعة في تهيئة المتجر لمحركات البحث. "
                     "يوصى بإعادة كتابة البيانات الوصفية وإسناد نصوص بديلة وصفية لجميع صور المحتوى.")
        elif score < 80:
            close = ("المتجر مهيأ جزئياً، ومعالجة البنود أعلاه من شأنها رفع درجة التوافق وتحسين فرص الظهور.")
        else:
    ...
    if weak:
        points.append(f"{weak} images have non-descriptive or duplicated alt text.")
    if crit_t:
        points.append(f"{crit_t} meta titles need urgent work (missing, too short, or too long).")
    if crit_d:
        points.append(f"{crit_d} meta descriptions need urgent work.")
    if not points:
    ...
    close = "Addressing the items above would raise the compliance score and improve search visibility."
    return intro, points, close
...
def generate_client_pdf(domain, score, stats, lang='ar'):
    ...
    if rtl:
        ...
        bold_target = str(FONT_BOLD_PATH) if (FONT_BOLD_PATH and Path(FONT_BOLD_PATH).exists()) else str(FONT_PATH)
        pdf.add_font(AR_FONT_NAME, "B", bold_target)
        if use_shaping:
    ...
    pdf.table([
        ...
        ('الصفحات التعريفية والسياسات' if rtl else 'Info and policy pages',
         f"{stats.get('info_pages', 0)} {T['u_page']}", 'neutral'),
        ...
        ('صفحات تعذّر الاتصال بها أثناء الفحص' if rtl else 'Pages unreachable during the scan',
    ...
    ok_t = tot - stats.get('bad_titles', 0)
    ok_d = tot - stats.get('bad_descs', 0)
    pdf.table([
        ('عناوين ضمن الطول المثالي (50-60)' if rtl else 'Titles within optimal length (50-60)',
        ...
        ('عناوين مجرد رموز أو قيمة قالب' if rtl else 'Titles that are symbols or placeholders',
        ...
        ('أوصاف ضمن الطول المثالي (120-150)' if rtl else 'Descriptions within optimal length (120-150)',
    ...
    pdf.table([
        ...
        ('صفحات بنفس العنوان والوصف حرفياً' if rtl else 'Pages with identical title and description',
    ...
    pdf.table([
        ('إجمالي صور المحتوى والمنتجات الفريدة' if rtl else 'Total content and product images',
        ...
        ('صور بنص بديل غير وصفي أو مكرر' if rtl else 'Images with non-descriptive or duplicated alt text',
        ...
        ('نسبة الصور بالصيغ الحديثة الخفيفة' if rtl else 'Share of modern lightweight formats',
    ...
    if stats.get('coverage_enabled'):
        ...
        pdf.table([
            ...
            ('روابط محذوفة ما زالت في الخريطة' if rtl else 'Deleted URLs still listed in the sitemap',
             f"{stats.get('dead_count', 0)} {T['u_link']}",
             'bad' if stats.get('dead_count') else 'ok'),
            ('صفحات في الخريطة لا يصل إليها الزائر' if rtl else 'Sitemap pages with no internal link',
             f"{stats.get('hidden_count', 0)} {T['u_page']}",
             'warn' if stats.get('hidden_count') else 'ok'),
            ('منتجات لا تظهر روابطها إلا بالتمرير' if rtl else 'Products linked only via scrolling',
             f"{stats.get('scroll_only_count', 0)} {T['u_product']}",
             'warn' if stats.get('scroll_only_count') else 'ok'),
            ('صفحات معروضة وغير مدرجة في الخريطة' if rtl else 'Live pages missing from the sitemap',
             f"{stats.get('unlisted_count', 0)} {T['u_page']}",
             'bad' if stats.get('unlisted_count') else 'ok'),
            ('نسبة المنتجات المعروضة المدرجة' if rtl else 'Visible products included in sitemap',
...
def generate_invoice_pdf(domain, quote, lang='ar', store_name=''):
    ...
    def money(value, x, w, y, size=10, bold=False, color=C_MUTED, center=True):
        """يكتب المبلغ مع وضع رمز الريال على يسار السعر مباشرة."""
        txt = f"{value:,.0f}"
        ...
        if rtl:
            # وضع رمز الريال على يسار السعر
            if riyal:
                try:
                    pdf.image(str(RIYAL_PATH), x=start,
                              y=y + (size * 0.5 - sym_h) / 2 + 0.2, h=sym_h)
                except Exception:
                    pass
                pdf.set_xy(start + sym_w + gap, y)
                pdf.cell(tw, size * 0.5, txt, 0, 0, 'L')
            else:
                curr_w = pdf.get_string_width(T['currency'] + " ")
                pdf.set_xy(start, y)
                pdf.cell(curr_w, size * 0.5, fmt(T['currency']), 0, 0, 'L')
                pdf.set_xy(start + curr_w, y)
                pdf.cell(tw, size * 0.5, txt, 0, 0, 'L')
        else:
            pdf.set_xy(start, y)
            pdf.cell(tw, size * 0.5, txt, 0, 0, 'L')
