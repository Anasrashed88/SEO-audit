...
from audit_engine import TITLE_MIN_OK, TITLE_MAX, TITLE_MIN_OPTIMAL, DESC_MIN_OK, DESC_MAX, DESC_MIN_OPTIMAL
...
DEFAULT_PRICES = {'meta_title': 15.0, 'meta_desc': 10.0, 'image_alt': 3.0}
PAYMENT = {'iban': 'SA87 1000 0026 5571 0000 0103', 'stc': '+966 55 354 1890'}
...
def generate_client_pdf(domain, score, stats, lang='ar'):
    ...
    class Report(FPDF):
        ...
        def footer(self):
            ...
            self.cell(W / 2, 8, fmt(T['page_of'].format(a=self.page_no())), align="R" if rtl else "L")
...
ضع هذا الكود داخل app.py (الملف الرئيسي لتشغيل تطبيق Streamlit):
...
with st.sidebar:
    ...
    do_pagination = st.checkbox("متابعة ترقيم الأقسام", value=True, help="يلتقط المنتجات في الصفحات التالية من كل قسم.")
    do_sitemap_check = st.checkbox("مقارنة مع خريطة الموقع", value=True, help="تشخيصية فقط — لا تؤثر على أرقام الفحص.")
    st.markdown("---")
...
if nav == "🔍 فحص متجر جديد":
    ...
    with c1:
        input_url = st.text_input("رابط المتجر الإلكتروني", value=st.session_state.current_url, placeholder="https://example.store")
    with c2:
        st.write("")
        st.write("")
    ...
    if st.session_state.audit_df is not None:
        if st.sidebar.button("🔄 تفريغ الشاشة", use_container_width=True):
            for k in ['audit_df', 'images_df', 'summary', 'coverage', 'selfcheck', 'dup_groups', 'structured']:
        ...
        with st.status("جارٍ الفحص...", expanded=True) as status:
            ...
            def progress(stage, **kw):
                if stage == 'discover_start':
                    ...
                    note.caption(f"الجولة {kw.get('round')} · فُحصت {done} صفحة · {pend} رابط في الانتظار")
                    ...
                    note.caption(f"{kw.get('i')}/{kw.get('total')} قسم · {kw.get('found')} منتج إضافي · {kw.get('fetched')} صفحة مجلوبة")
                elif stage == 'extra_start':
                    head.write(f"**المرحلة 3** — فحص {kw.get('count')} منتج من الصفحات التالية")
                    bar.progress(0)
                elif stage == 'retry_start':
                    head.write(f"**إعادة محاولة** — {kw.get('count')} صفحة تعذّر الاتصال بها")
                    bar.progress(0)
                elif stage == 'done':
                    bar.progress(1.0)

            result = run_full_scan(target, max_pages, workers, do_pagination, do_sitemap_check, progress)
    ...
    if st.session_state.audit_df is not None:
        ...
        st.markdown(
            ...
            f'<div class="plat">المنصة: {plat_txt} · {datetime.now().strftime("%Y-%m-%d")}</div></div></div>',
            unsafe_allow_html=True)

        if platform not in SUPPORTED_PLATFORMS:
            st.warning("لم يتم التعرف على المنصة كسلة أو زد أو شوبيفاي. الأداة معايرة على هذه المنصات الثلاث — راجع تبويب التحقق اليدوي قبل إرسال التقرير لأي عميل.")
        ...
        cards = [
            ...
            (summary["blog_pages"], "المدونة", COLOR['warn'] if summary["blog_pages"] == 0 else COLOR['accent']),
            (summary["info_pages"], "التعريفية", COLOR['accent']),
            (summary["missing_alts"], "صور بلا Alt", COLOR['bad'] if summary["missing_alts"] else COLOR['ok']),
            (summary.get("broken_pages", 0), "روابط معطلة", COLOR['bad'] if summary.get("broken_pages") else COLOR['ok']),
        ]
        ...
        vmap = {'ready': ('v-ready', '✅ جاهز للإرسال', 'اجتازت النتائج كل فحوص الموثوقية. يمكنك إرسال التقرير.'),
                'review': ('v-review', '⚠️ يحتاج مراجعة قبل الإرسال', 'بعض المؤشرات تحتاج نظرة سريعة منك قبل إرسال التقرير لعميل.'),
                'blocked': ('v-blocked', '⛔ لا ترسل هذا التقرير', 'رصدت الأداة خللاً يجعل أرقامها غير موثوقة لهذا المتجر.')}
        if selfcheck:
            cls, head, sub = vmap[selfcheck['verdict']]
            extra = (f" · {selfcheck['fails']} فحص فاشل" if selfcheck['fails'] else "")
            extra += (f" · {selfcheck['warns']} تنبيه" if selfcheck['warns'] else "")
            st.markdown(f'<div class="verdict {cls}"><b>{head}</b>{extra}<br>'
                        f'<span style="font-size:13.5px;font-weight:400">{sub} التفاصيل في تبويب «فحص الثقة».</span></div>',
                        unsafe_allow_html=True)
        ...
        with tabs[0]:
            ...
            with g1:
                ...
                st.markdown(bar_chart("توزيع أنواع الصفحات", items), unsafe_allow_html=True)
            ...
            if 'جودة الرابط' in ok.columns:
                ...
                items = [(UL[k], int(uc.get(k, 0))) for k in
                         ['u_ok', 'u_underscore', 'u_uppercase', 'u_repeat', 'u_wordy',
                          'u_long', 'u_generic', 'u_malformed', 'u_wrongname', 'u_clone'] if uc.get(k, 0)]
                if len(items) > 1:
                    st.markdown(bar_chart("جودة روابط الصفحات", items, {
                        ...
                        UL['u_wrongname']: COLOR['bad'], UL['u_clone']: COLOR['bad']}),
                        unsafe_allow_html=True)
            if summary.get('img_formats'):
                items = [(k.upper(), v) for k, v in sorted(summary['img_formats'].items(), key=lambda x: -x[1])[:5]]
                st.markdown(bar_chart("صيغ صور المتجر", items, {
                    'WEBP': COLOR['ok'], 'AVIF': COLOR['ok'],
                    'JPG': COLOR['warn'], 'PNG': COLOR['warn']}), unsafe_allow_html=True)

            st.markdown("#### أبرز النتائج")
            out = []
            if summary['missing_alts']:
                out.append((f"<b>{summary['missing_alts']}</b> صورة بلا نص بديل إطلاقاً من أصل {summary['total_images']} — أكبر فجوة قابلة للإصلاح في المتجر.", 'bad'))
            if summary['weak_alts']:
                out.append((f"<b>{summary['weak_alts']}</b> صورة نصها البديل موجود لكنه غير وصفي أو مكرر — يمر كسليم في الأدوات السطحية.", 'warn'))
            if summary.get('url_clone'):
                out.append((f"<b>{summary['url_clone']}</b> منتج مستنسخ — نسخ مكررة من منتج واحد تتنافس مع أصلها.", 'bad'))
            if summary.get('dup_content'):
                out.append((f"<b>{summary['dup_content']}</b> صفحة بنفس العنوان والوصف حرفياً — محتوى مكرر يختار جوجل منه واحدة فقط.", 'bad'))
            if summary.get('url_wrongname'):
                out.append((f"<b>{summary['url_wrongname']}</b> رابط يحمل اسم منتج مختلف عن المعروض في الصفحة — الزبون ينقر على منتج ويصل لآخر.", 'bad'))
            if summary.get('url_style'):
                out.append((f"<b>{summary['url_style']}</b> رابط بصياغة غير مثالية — شرطة سفلية أو حروف كبيرة أو طول مفرط.", 'warn'))
            if summary.get('img_legacy') and summary.get('img_modern_pct', 100) < 50:
                out.append((f"<b>{summary['img_legacy']}</b> صورة بصيغة قديمة ثقيلة — {summary.get('img_modern_pct', 0)}% فقط بصيغ حديثة. التحويل يخفض حجم الصفحة بنحو الثلث.", 'warn'))
            if summary.get('title_symbols'):
                out.append((f"<b>{summary['title_symbols']}</b> عنوان ميتا مجرد رموز أو قيمة قالب افتراضية — الصفحة بلا عنوان فعلي في نتائج البحث.", 'bad'))
            if summary.get('title_brand_only'):
                out.append((f"<b>{summary['title_brand_only']}</b> عنوان لا يحمل سوى اسم المتجر بلا وصف للمنتج.", 'bad'))
            if summary.get('title_dup'):
                out.append((f"<b>{summary['title_dup']}</b> صفحة تتشارك نفس عنوان الميتا.", 'warn'))
            if summary.get('desc_same'):
                out.append((f"<b>{summary['desc_same']}</b> وصف ميتا نسخة حرفية من العنوان.", 'warn'))
            if summary['critical_titles']:
                out.append((f"<b>{summary['critical_titles']}</b> عنوان مفقود أو قصير جداً أو يُقتطع في نتائج البحث.", 'bad'))
            if summary['critical_descs']:
                out.append((f"<b>{summary['critical_descs']}</b> وصف ميتا خارج الحدود المفيدة.", 'warn'))
            if summary['canon_missing']:
                out.append((f"<b>{summary['canon_missing']}</b> صفحة بلا وسم كانونيكال — خطر تكرار المحتوى.", 'warn'))
            if summary.get('hidden_count'):
                bt = summary.get('orphan_by_type', {})
                brk = "، ".join(f"{PAGE_TYPE_LABEL['ar'].get(k, k)}: {v}" for k, v in bt.items())
                out.append((f"<b>{summary['hidden_count']}</b> صفحة يتيمة في خريطة الموقع ({brk}) — منشورة ولا يصل إليها أحد.", 'warn'))
            if summary.get('redirect_count'):
                out.append((f"<b>{summary['redirect_count']}</b> رابط في الخريطة يعيد التوجيه لصفحة أخرى — هدر لميزانية الزحف.", 'warn'))
            if summary['broken_pages']:
                out.append((f"<b>{summary['broken_pages']}</b> رابط معطل يصل إليه الزائر.", 'bad'))
            if summary['thin_pages']:
                out.append((f"<b>{summary['thin_pages']}</b> صفحة بمحتوى نصي ضعيف (أقل من 50 كلمة).", 'warn'))
            if not out:
        ...
        with tabs[1]:
            ...
            st.caption("هذه الفحوص لا تقيس جودة سيو المتجر، بل تقيس مدى ثقة الأداة في أرقامها هي.")
            if not selfcheck:
                st.info("لا توجد نتائج فحص ذاتي لهذه الجلسة.")
            else:
                dots = {CHECK_PASS: COLOR['ok'], CHECK_WARN: COLOR['warn'], CHECK_FAIL: COLOR['bad']}
                order = {CHECK_FAIL: 0, CHECK_WARN: 1, CHECK_PASS: 2}
                for c in sorted(selfcheck['checks'], key=lambda x: order[x['level']]):
                    act = f'<div class="a">الإجراء المقترح: {c["action"]}</div>' if c['action'] else ''
                    st.markdown(
                        f'<div class="chk"><div class="dot" style="background:{dots[c["level"]]}"></div>'
        ...
        with tabs[2]:
            ...
            st.caption("تُقارَن كل صفحة قسم بعدّاد المنتجات الظاهر فيها.")
            if not stx:
                ...
                if not checked:
                    st.markdown(finding("لم يعرض المتجر عدّاد منتجات في أقسامه، فتعذّرت مقارنة التغطية.", 'warn'), unsafe_allow_html=True)
                elif short:
                    st.markdown(finding(f"<b>{len(short)}</b> قسماً يعلن منتجات أكثر مما رصده الفحص (ناقص <b>{miss}</b> منتجاً).", 'bad'), unsafe_allow_html=True)
                else:
                    st.markdown(finding(f"جميع الأقسام الـ{checked} مطابقة تماماً لعدّاداتها.", 'ok'), unsafe_allow_html=True)

                rows = stx.get('category_rows') or []
                if rows:
                    st.dataframe(pd.DataFrame(rows), use_container_width=True,
                                 column_config={"القسم": st.column_config.LinkColumn("القسم", width="large")})

                gaps = stx.get('image_gap') or []
                if gaps:
                    st.markdown(finding(f"<b>{len(gaps)}</b> صفحة منتج تعلن صوراً أكثر مما رصده الفحص.", 'bad'), unsafe_allow_html=True)
                    st.dataframe(pd.DataFrame(gaps), use_container_width=True)

                nm = stx.get('name_mismatch') or []
                if nm:
                    st.markdown(finding(f"<b>{len(nm)}</b> منتجاً اسمه المعلن يختلف عن المعروض.", 'warn'), unsafe_allow_html=True)
                    st.dataframe(pd.DataFrame(nm), use_container_width=True)

        with tabs[3]:
            present = [PAGE_TYPE_LABEL['ar'][t] for t in PAGE_TYPE_ORDER if (df['نوع الصفحة'] == t).any()]
            ...
            if only_issues:
                ...
                d = d[(d['حالة العنوان'] != STATUS_LABEL['ar']['optimal']) |
                      ...
                      (~d['جودة الرابط'].isin([URL_LABEL['ar']['u_ok'], URL_LABEL['ar']['u_na']])) |
                      (d['صور بدون Alt'] > 0) | (d['متاحة'] == False)]  # noqa: E712
            d = d.copy().reset_index(drop=True)
            d.index = d.index + 1
            st.dataframe(d, use_container_width=True, height=460,
                         column_config={"الرابط": st.column_config.LinkColumn("الرابط", width="large"),
                                        "درجة السيو": st.column_config.ProgressColumn("درجة السيو", min_value=0, max_value=100, format="%d")})
        ...
        with tabs[4]:
            if view_imgs is not None and not view_imgs.empty:
                ...
                with fc1:
                    f = st.selectbox("تصفية", ["الكل", "بلا نص بديل", "نص بديل ضعيف", "نص بديل سليم"])
                with fc2:
                    fmts = ["الكل"] + sorted(view_imgs['صيغة الصورة'].dropna().unique().tolist()) if 'صيغة الصورة' in view_imgs.columns else ["الكل"]
                    fsel = st.selectbox("صيغة الصورة", fmts)
                v = view_imgs
                if f == "بلا نص بديل":
                    v = view_imgs[view_imgs['حالة النص البديل'] == L['alt_missing']]
                elif f == "نص بديل ضعيف":
                    v = view_imgs[view_imgs['حالة النص البديل'].isin([L[k] for k in ALT_WEAK_STATES])]
                elif f == "نص بديل سليم":
                ...
                st.dataframe(v, use_container_width=True, height=460,
                             column_config={"رابط الصورة": st.column_config.ImageColumn("معاينة", width="small"),
                                            "رابط الصفحة": st.column_config.LinkColumn("رابط الصفحة", width="medium")})
            else:
        ...
        with tabs[5]:
            if not coverage:
                ...
                if coverage['dead_pages']:
                    st.markdown(finding(f"<b>{len(coverage['dead_pages'])}</b> رابط محذوف ما زال معلناً في خريطة الموقع.", 'bad'), unsafe_allow_html=True)
                    st.dataframe(localize_df(pd.DataFrame(coverage['dead_pages']), 'ar'), use_container_width=True)

                if coverage['unlisted_pages']:
                    st.markdown(finding(f"<b>{len(coverage['unlisted_pages'])}</b> صفحة معروضة وغير مدرجة في خريطة الموقع.", 'bad'), unsafe_allow_html=True)
                    st.dataframe(localize_df(pd.DataFrame(coverage['unlisted_pages']), 'ar'), use_container_width=True)

                if coverage.get('scroll_only_products'):
                    with st.expander(f"عرض الـ{len(coverage['scroll_only_products'])} منتجاً يظهر بالتمرير فقط"):
                        st.dataframe(localize_df(pd.DataFrame(coverage['scroll_only_products']), 'ar'), use_container_width=True)

                if coverage['orphan_pages']:
                    st.markdown(finding(f"<b>{len(coverage['orphan_pages'])}</b> صفحة يتيمة مدرجة ولا يصلها الزائر برابط داخلي.", 'warn'), unsafe_allow_html=True)
                    st.dataframe(localize_df(pd.DataFrame(coverage['orphan_pages']), 'ar'), use_container_width=True)
        ...
        with tabs[7]:
            ...
            exp_summary['platform_label'] = (PLATFORM_LABEL if lang == 'ar' else PLATFORM_LABEL_EN).get(platform, '—')
            netloc = urlparse(st.session_state.current_url).netloc or "store"

            try:
                pdf_bytes = generate_client_pdf(st.session_state.current_url, summary['score'], exp_summary, lang)
            except Exception as e:
                pdf_bytes = None
                st.error(f"تعذر توليد الـ PDF: {e}")

            zip_bytes = build_zip(df, images_df, coverage, lang, st.session_state.get('structured'))

            gate_ok = True
            if selfcheck and selfcheck['verdict'] == 'blocked':
                st.markdown('<div class="verdict v-blocked"><b>⛔ تقرير العميل محجوب</b><br>'
                            '<span style="font-size:13.5px;font-weight:400">رصدت الأداة خللاً يجعل أرقامها غير موثوقة لهذا المتجر. راجع تبويب «فحص الثقة» أولاً.</span></div>', unsafe_allow_html=True)
                gate_ok = st.checkbox("راجعت الفحوص الفاشلة وأتحمل مسؤولية إرسال هذا التقرير", value=False)
            elif selfcheck and selfcheck['verdict'] == 'review':
                st.warning("توجد تنبيهات على موثوقية النتائج. راجع تبويب «فحص الثقة» قبل إرسال التقرير لعميل.")

            st.markdown("---")
            st.markdown("##### 💰 عرض السعر")
            pc1, pc2, pc3 = st.columns(3)
            with pc1:
                p_title = st.number_input("سعر عنوان الميتا + الرابط (ريال)", 1.0, 200.0, DEFAULT_PRICES['meta_title'], 1.0)
            with pc2:
                p_desc = st.number_input("سعر وصف الميتا (ريال)", 1.0, 200.0, DEFAULT_PRICES['meta_desc'], 1.0)
            with pc3:
                p_alt = st.number_input("سعر وصف الصورة (ريال)", 0.5, 100.0, DEFAULT_PRICES['image_alt'], 0.5)

            dc1, dc2 = st.columns([1, 3])
            with dc1:
                use_disc = st.checkbox("إضافة خصم", value=False)
            with dc2:
                disc_pct = st.slider("نسبة الخصم %", 0, 50, 10, disabled=not use_disc)

            quote = build_quote(summary, {'meta_title': p_title, 'meta_desc': p_desc, 'image_alt': p_alt}, discount_rate=(disc_pct / 100 if use_disc else 0.0))
            qc = st.columns(4)
            qc[0].metric("عناوين وروابط", f"{summary.get('bad_titles', 0)}")
            qc[1].metric("أوصاف ميتا", f"{summary.get('bad_descs', 0)}")
            qc[2].metric("صور تحتاج وصفاً", f"{summary.get('missing_alts', 0) + summary.get('weak_alts', 0)}")
            qc[3].metric("الإجمالي المستحق", f"{quote['total']:,.0f}")

            try:
                inv_bytes = generate_invoice_pdf(st.session_state.current_url, quote, lang)
            except Exception as e:
                inv_bytes = None
                st.error(f"تعذّر توليد الفاتورة: {e}")

            if inv_bytes:
                st.download_button("🧾 تحميل عرض السعر (PDF)" if lang == 'ar' else "🧾 Download quotation (PDF)",
                                   inv_bytes, f"Quote_{netloc}_{lang}.pdf", "application/pdf", use_container_width=True)
            st.markdown("---")

            d1, d2 = st.columns(2)
            with d1:
                if pdf_bytes and gate_ok:
                    st.download_button("📄 تقرير العميل (PDF)" if lang == 'ar' else "📄 Client report (PDF)",
                                       pdf_bytes, f"SEO_Audit_{netloc}_{lang}.pdf", "application/pdf", use_container_width=True)
                elif pdf_bytes:
                    st.button("📄 تقرير العميل (PDF)", disabled=True, use_container_width=True, help="مُعطّل حتى تقرّ بمراجعة الفحوص الفاشلة.")
            with d2:
                st.download_button("📦 حزمة البيانات (ZIP)" if lang == 'ar' else "📦 Data package (ZIP)",
                                   zip_bytes, f"Data_Package_{netloc}_{lang}.zip", "application/zip", use_container_width=True)
    ...
    if hist.empty:
        ...
        sel_id = st.selectbox(
            ...
            format_func=lambda x: f"{hist[hist['id']==x]['المتجر'].values[0]} ({hist[hist['id']==x]['تاريخ الفحص'].values[0]})")
        if st.button("📥 استرجاع البيانات", type="primary"):
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            c.execute("SELECT domain, data_json, images_json, coverage_json, platform FROM audits WHERE id = ?", (sel_id,))
            row = c.fetchone()
            conn.close()
            if row:
                ...
                st.session_state.summary['platform_label'] = PLATFORM_LABEL.get(row[4] or 'unknown', '—')
